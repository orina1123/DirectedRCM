...
int LoadEmb(string modelname) {
    ...
    for (b = 0; b < words; b++) {
        ...
        *****/
        ...
        /*****/
        for (a = 0; a < size; a++) fread(&syn0[a + id * size], sizeof(real), 1, f);
        for (a = 0; a < size; a++) fread(&syn1neg[a + id * size], sizeof(real), 1, f);
        /*****/
    }
    fclose(f);
...
//        //TrainModelRegNCEThread(0);
    ...
    for (a = num_threads; a < num_threads + num_thread_pmm; a++) pthread_join(pt[a], NULL);
